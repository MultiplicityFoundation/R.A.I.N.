...
DEFAULT_PAPER_TITLE_ALLOWLIST = _parse_env_csv(
    "RAIN_PAPER_TITLE_ALLOWLIST",
    (
        "Coherence Depth",
        "Discrete Celestial Holography",
        "Dynamic Resonance Rooting (Friction Mechanism)",
        "Location is a Dynamic Variable",
        "Reality Built on a Finite Set of Geometric Instructions",
        "Recursive Resonance Stabilization",
...
class Config:
    ...
    session_runs_path: str = os.environ.get("RAIN_SESSION_RUNS", "meeting_archives/session_runs.jsonl")
    ...
    # Context Settings
    context_snippet_length: int = int(os.environ.get("RAIN_CONTEXT_SNIPPET_LENGTH", "1500"))
    total_context_length: int = int(os.environ.get("RAIN_TOTAL_CONTEXT_LENGTH", "8000"))
    ...
    paper_title_allowlist: Tuple[str, ...] = DEFAULT_PAPER_TITLE_ALLOWLIST
