...
def run_simulation(name):
    """Runs a Vers3Dynamics simulation. Options: 'gravmag', 'ions', 'rlc', 'reactor'"""
    if not hello_os:
        return "Error: hello_os module not found in library path."
    
    print(f"🚀 LAUNCHING SIMULATION: {name}...")
    if name == 'gravmag':
        hello_os.run_gravmag()
        return "Gravitomagnetic Rotor Explorer launched in separate window."
    elif name == 'ions':
        hello_os.run_ions_x()
        return "IONS_X Field Simulation launched in separate window."
    elif name == 'rlc':
        hello_os.run_rlc()
        return "RLC Explorer launched in separate window."
    elif name == 'reactor':
        hello_os.run_reactor_swarm()
        return "Reactor Physics Swarm launched (check console output)."
    else:
        return f"Unknown simulation '{name}'. Options: gravmag, ions, rlc, reactor"
...
status = run_simulation("name")          # Run 'gravmag', 'ions', 'rlc', or 'reactor'
```
...
- Only use: read_paper(), search_web(), list_papers(), search_library(), semantic_search(), run_simulation()
"""
...
def create_team() -> List[Agent]:
    ...
    return [
        Agent(
            ...
            color="\033[92m",  # Green
            tool_instruction="""
AVAILABLE: read_paper(), search_web(), list_papers(), run_simulation()
...
6. run_simulation(name): Run 'gravmag', 'ions', 'rlc', or 'reactor'.
