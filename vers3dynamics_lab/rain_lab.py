...
def _print_banner() -> None:
    ...
    for line, color in zip(BANNER_LINES, colors):
        ...
        print(f"\033[A{color}{line}{ANSI_RESET}")
...
def parse_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Chat mode only: LM request timeout in seconds (maps to --timeout). 0 or -1 for infinite.",
    )
    parser.add_argument(
        "--recursive-depth",
        type=int,
    ...
    parser.add_argument(
        "--no-voice",
        action="store_true",
        help="Disable TTS output (passed to chat mode)",
    )
