...
# Optional: Graph-R1 hypergraph support
try:
    from graph_bridge import HypergraphManager, GRAPH_AVAILABLE
except ImportError:
    GRAPH_AVAILABLE = False
    HypergraphManager = None
...
# --- VOICE MANAGER ---
class VoiceManager:
    """Handles text-to-speech output for agent responses with per-character Windows voices.
    
    IMPORTANT: On Windows, pyttsx3's COM/SAPI backend hangs if you reuse
    a single engine across multiple runAndWait() calls with voice changes.
    Fix: create a FRESH engine for every speak() call and destroy it after.
    """

    def __init__(self):
        self.david_voice_id = None
        self.zira_voice_id = None

        if VOICE_AVAILABLE:
            # One-time voice discovery using a temporary engine
            engine = None
            try:
                engine = pyttsx3.init()
                ...
                for i, voice in enumerate(available_voices):
                    ...
                    if 'david' in vid:
                        self.david_voice_id = voice.id
                    elif 'zira' in vid:
                        self.zira_voice_id = voice.id
                
                # Fallback: if no David/Zira, just take index 0 and 1
                if not self.david_voice_id and len(available_voices) > 0:
                    self.david_voice_id = available_voices[0].id
                if not self.zira_voice_id and len(available_voices) > 1:
                    self.zira_voice_id = available_voices[1].id
                
                print(f"   -> Male/Default Voice: {'David (Found)' if 'david' in str(self.david_voice_id).lower() else 'Fallback (Index 0)'}")
                print(f"   -> Female/Alt Voice: {'Zira (Found)' if 'zira' in str(self.zira_voice_id).lower() else 'Fallback (Index 1)'}")

            except Exception as e:
                print(f"⚠️  Voice engine init failed: {e}")
            finally:
                # Destroy discovery engine — never reuse it
                if engine:
                    try:
                        engine.stop()
                    except:
    ...
    def _get_voice_settings(self, agent_name: str):
        """Return (voice_id, rate) for a given agent."""
        # Select Voice ID
        if agent_name in ("James", "Luca"):
            voice_id = self.david_voice_id
        elif agent_name in ("Jasmine", "Elena"):
            voice_id = self.zira_voice_id or self.david_voice_id
        else:
            voice_id = self.david_voice_id

        # Select Rate
        rates = {"James": 160, "Jasmine": 185, "Luca": 195, "Elena": 210}
        rate = rates.get(agent_name, 175)

        return voice_id, rate

    def speak(self, text: str, agent_name: str):
        """Speak the given text using a FRESH engine each time (avoids COM hangs)."""
        if not VOICE_AVAILABLE:
            return
        
        print(f"🔊 [{agent_name} speaking...]")
        engine = None
        try:
            engine = pyttsx3.init()
            voice_id, rate = self._get_voice_settings(agent_name)
            if voice_id:
            ...
            print(f"⚠️  Voice error for {agent_name}: {e}")
        finally:
            if engine:
                try:
                    engine.stop()
...
class Config:
    ...
    timeout: float = float(os.environ.get("RAIN_LM_TIMEOUT", "600"))
    max_retries: int = 1
    ...
    max_turns: int = 25
    wrap_up_turns: int = 15  # Wrap-up phase lasts this many turns (discussion ends at max_turns - wrap_up_turns)
    recent_history_window: int = 5   # Verbatim turns visible to each agent
    summary_interval: int = 4        # Summarize older turns every N turns
    enable_meeting_summary: bool = True  # Inject rolling summary into system message
    
    # Context Settings - EXPANDED FOR DEEPER PAPER ANALYSIS
    context_snippet_length: int = 8000    # ~2k tokens per paper - deep content coverage
    total_context_length: int = 28000     # ~7k tokens for papers - fits comfortably in 32K context
    max_resume_context_length: int = 2000   # Max chars to load from previous meeting log
    ...
    # Web Search Settings
    enable_web_search: bool = False  # Enable DuckDuckGo search for online context
    ...
    enable_voice: bool = True  # Enable TTS output
...
class ContextManager:
    ...
    def _discover_files(self) -> List[Path]:
        ...
        def _looks_like_paper_title(name: str) -> bool:
            # "Obvious title names" typically contain spaces/parentheses (e.g., "Discrete Celestial Holography").
            # This rejects repo/docs like "CONTRIBUTING.md" and most code-adjacent files.
            stem = Path(name).stem
            return (" " in stem) or ("(" in stem) or (")" in stem)

        filtered = [
            f for f in candidates
            if not f.name.startswith("_")
            and not any(x in f.name.upper() for x in deny)
            and _looks_like_paper_title(f.name)
        ]
    ...
    def get_agent_context(self, agent: "Agent", topic: str, budget: int = None) -> Tuple[str, List[str]]:
        """Route the most relevant papers to a specific agent based on topic + specialty keywords.
        
        Returns (context_block, list_of_routed_paper_names).
        Falls back to full context if no papers score above threshold.
        """
        if not self.loaded_papers:
            return "No research papers loaded.", []

        if budget is None:
            budget = self.config.total_context_length

        # Build keyword set from topic + agent focus
        def _keywords(text: str) -> set:
            stop = {"the","a","an","of","in","to","and","or","is","are","for","on","with","as","at","by","from"}
            return {w.lower() for w in re.split(r'\W+', text) if len(w) > 2 and w.lower() not in stop}

        query_keywords = _keywords(topic) | _keywords(agent.focus)

        # Score each paper by keyword hit count (normalized by paper length)
        scored = []
        for paper_name, content in self.loaded_papers.items():
            content_lower = content.lower()
            hits = sum(1 for kw in query_keywords if kw in content_lower)
            # Normalize: hits per 1000 chars so shorter papers aren't unfairly penalized
            score = hits / max(len(content) / 1000.0, 1.0)
            scored.append((score, paper_name, content))

        # Sort descending by score
        scored.sort(key=lambda x: x[0], reverse=True)

        # Accept papers above a minimum threshold (at least 1 keyword hit)
        threshold = 0.0  # anything with ≥1 hit
        qualifying = [(name, content) for score, name, content in scored if score > threshold]

        # Fall back to all papers if nothing qualifies
        if not qualifying:
            qualifying = [(name, content) for _, name, content in scored]

        # Fill budget greedily with highest-scoring papers
        buffer = []
        routed_names = []
        total_chars = 0
        for paper_name, content in qualifying:
            remaining = budget - total_chars
            if remaining < 500:
                break
            snippet = content[:min(len(content), self.config.context_snippet_length, remaining)]
            buffer.append(f"--- PAPER: {paper_name} ---\n{snippet}\n")
            total_chars += len(snippet)
            routed_names.append(paper_name)

        return "\n".join(buffer), routed_names
...
class RainLabDirector:
    ...
    def get_dynamic_instruction(self, agent: Agent, turn_count: int, topic: str) -> str:
        """Generate instructions that force citation from MULTIPLE papers"""
        
        # Build a paper hint string so agents know what's available
        paper_hint = ", ".join(self.paper_list[:6]) if self.paper_list else "the loaded papers"
        
        # Opening move
        if turn_count == 0 and agent.name == "James":
            return (
                f"Open the meeting. Survey ALL loaded research papers and identify "
                f"which ones discuss '{topic}'. Quote key findings from AT LEAST TWO "
                f"different papers to set the stage. Available papers include: {paper_hint}"
            )
        ...
        if turn_count == 4 and self.paper_list:
            ...
            if agent.name == "James":
                return (
                    f"Focus specifically on '{random_paper}'. What does it say about '{topic}'? "
                    f"Quote directly, then connect it to a finding from a DIFFERENT paper."
                )
        
        # Research-Specific Instructions — each agent has cross-paper tasks
        instructions = {
            "James": [
                f"Quote a specific finding about '{topic}' from one paper, then show how a SECOND paper supports or contradicts it.",
                f"Synthesize: How do MULTIPLE papers relate to '{topic}'? Reference at least two by name.",
                f"What is the core innovation regarding '{topic}'? Quote from one paper, then cite corroborating evidence from another.",
                f"Find and compare TWO different mentions of '{topic}' from different papers. Which aligns better with our goals?",
                f"Connect the dots: pick two papers from [{paper_hint}] and explain how they jointly inform '{topic}'.",
            ],
            "Jasmine": [
                f"Critique implementation: What do MULTIPLE papers say about building '{topic}'? Quote constraints from at least two sources.",
                f"Do the papers mention energy/hardware requirements for '{topic}'? Compare specs across papers.",
                f"Find experimental setups in the text related to '{topic}'. Compare parameters from different papers.",
                f"What materials or components are mentioned for '{topic}'? Cross-reference across at least two papers.",
                f"One paper proposes X for '{topic}' — does another paper's data support or challenge that? Quote both.",
            ],
            "Luca": [
                f"Describe the theoretical geometry of '{topic}' using equations from the text. Do multiple papers agree on the formalism?",
                f"Visualize '{topic}' using descriptions from the papers. Combine imagery from at least two sources.",
                f"What topology or structure defines '{topic}' in the text? Compare descriptions across papers.",
                f"Find field equations related to '{topic}' in two different papers. Are they consistent? Quote both.",
                f"Bridge the geometry: show how a concept in one paper maps onto a structure described in another.",
            ],
            "Elena": [
                f"Check mathematical consistency of '{topic}' ACROSS papers. Do they use compatible notation and assumptions?",
                f"Do papers define limits (bits, entropy, error) for '{topic}'? Compare numerical values across sources.",
                f"Compare '{topic}' in the text to standard QM. Do multiple papers reach the same conclusion? Quote differences.",
                f"Find information-theoretic bounds on '{topic}' in at least two papers. Are they consistent?",
                f"One paper claims X about '{topic}' — does the math in another paper support that claim? Audit both.",
            ]
        }
        ...
        return f"Analyze '{topic}' using evidence from MULTIPLE research papers. Quote your sources and name the papers."
...
# --- DIPLOMAT (FILE-BASED MAILBOX) ---
class Diplomat:
    """File-based inter-process communication: inbox/outbox for live meeting messages"""

    def __init__(self, library_path: str):
        self.base = os.path.join(library_path, "communication")
        self.inbox = os.path.join(self.base, "inbox")
        self.outbox = os.path.join(self.base, "outbox")
        self.processed = os.path.join(self.base, "processed")
        for d in (self.inbox, self.outbox, self.processed):
            os.makedirs(d, exist_ok=True)

    def check_inbox(self) -> Optional[str]:
        """Check for .txt files in inbox. Return content of the first one found, or None."""
        try:
            txt_files = sorted(glob.glob(os.path.join(self.inbox, "*.txt")))
            if not txt_files:
                return None
            ...
            # Move to processed
            shutil.move(filepath, os.path.join(self.processed, filename))

            return f"MESSAGE FROM OUTSIDE: {content}"
        except Exception as e:
            print(f"\u26a0\ufe0f  Diplomat inbox error: {e}")
            return None

    def send_reply(self, text: str):
        """Write a timestamped reply file into outbox."""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            outfile = os.path.join(self.outbox, f"reply_{timestamp}.txt")
            with open(outfile, "w", encoding="utf-8") as f:
                f.write(text)
        except Exception as e:
            print(f"\u26a0\ufe0f  Diplomat outbox error: {e}")
...
class RainLabOrchestrator:
    ...
    def __init__(self, config: Config):
        ...
        self.diplomat = Diplomat(config.library_path)
        self.meeting_summary = ""  # Rolling compressed summary of older turns
        
        # Optional hypergraph engine
        self.hypergraph = None
        if GRAPH_AVAILABLE and HypergraphManager is not None:
            self.hypergraph = HypergraphManager()
            self.hypergraph.build_graph(config.library_path, verbose=config.verbose)
        ...
        try:
            ...
            # If timeout is <= 0, we treat it as infinite (None in httpx)
            read_timeout = self.config.timeout if self.config.timeout > 0 else None
            
            # Increase connect timeout to handle initial model loading lag
    ...
    def _update_meeting_summary(self, history_log: List[str], topic: str) -> None:
        """Compress older turns into a rolling bullet-point summary via a lightweight LLM call.
        Updates self.meeting_summary in place. Called every summary_interval turns."""
        if not self.config.enable_meeting_summary or len(history_log) < self.config.summary_interval:
            return

        # Only summarize turns older than the recent window — keep recent ones verbatim
        turns_to_summarize = history_log[:-self.config.recent_history_window] if len(history_log) > self.config.recent_history_window else history_log
        if not turns_to_summarize:
            return

        # Include previous summary so it accumulates correctly
        prior = f"\nPrevious summary:\n{self.meeting_summary}\n\n" if self.meeting_summary else ""
        transcript = "\n".join(turns_to_summarize[-20:])  # cap at 20 older turns

        try:
            resp = self.client.chat.completions.create(
                model=self.config.model_name,
                messages=[
                    {"role": "system", "content": "You are a concise meeting minutes writer."},
                    {"role": "user", "content": (
                        f"Topic: {topic}\n{prior}"
                        f"New discussion to summarize:\n{transcript}\n\n"
                        "Write a concise summary of the key points raised so far, "
                        "as 4-5 short bullet points. Max 120 words total. "
                        "Focus on claims made, papers cited, and any disagreements."
                    )}
                ],
                temperature=0.3,
                max_tokens=180
            )
            self.meeting_summary = resp.choices[0].message.content.strip()
            print(f"\033[90m   📝 Meeting memory updated ({len(self.meeting_summary)} chars)\033[0m")
        except Exception as e:
            print(f"\033[90m   ⚠ Summary update failed: {e}\033[0m")
...
"""
        ...
        # Store web context separately — it will be appended per-agent after routing
        self.web_context = web_context
        # Reset session memory for this meeting
        self.meeting_summary = ""
        ...
        while turn_count < self.config.max_turns:
            ...
            # --- DIPLOMAT: check inbox for external messages ---
            incoming = self.diplomat.check_inbox()
            if incoming:
                print(f"\n\033[97m\U0001f4e8 [GUEST]: {incoming}\033[0m")
                self.log_manager.log_statement("GUEST", incoming)
                history_log.append(f"GUEST: {incoming}")
            ...
            intervention_window = 0.3  # seconds to wait for user input
            ...
            # 1b. Update rolling meeting summary every summary_interval turns
            if self.config.enable_meeting_summary and turn_count > 0 and turn_count % self.config.summary_interval == 0:
                self._update_meeting_summary(history_log, topic)

            # 1c. Route papers to this agent by relevance
            agent_papers, routed_names = self.context_manager.get_agent_context(current_agent, topic)
            if self.web_context:
                agent_papers = agent_papers + "\n\n" + self.web_context
            print(f"\033[90m   🗂 Routing {len(routed_names)} paper(s) to {current_agent.name}: {', '.join(routed_names[:3])}{'...' if len(routed_names) > 3 else ''}\033[0m")
            ...
            if self.config.enable_voice:
                print(f"\033[90m[DEBUG] TTS START: {current_agent.name}\033[0m")
                self.voice_manager.speak(clean_response, current_agent.name)
                print(f"\033[90m[DEBUG] TTS END\033[0m")
            else:
                print(f"\033[90m[DEBUG] TTS SKIPPED (Disabled)\033[0m")
            ...
            # 4b. Active Web Search — detect [SEARCH: ...] in response
            search_match = re.search(r'\[SEARCH:\s*(.+?)\]', response)
            if search_match and self.web_search_manager.enabled:
                search_query = search_match.group(1).strip()
                print(f"\n\033[93m🌐 Active Web Search triggered: \"{search_query}\"\033[0m")
                search_result, _ = self.web_search_manager.search(search_query, verbose=True)
                if search_result:
                    print(f"\033[90m{search_result[:500]}\033[0m")
                    history_log.append(f"SYSTEM [Web Search Result for '{search_query}']: {search_result[:800]}")
                else:
                    print("\033[90m   (no results)\033[0m")
                    history_log.append(f"SYSTEM [Web Search for '{search_query}']: No results found.")

            # 5. Log
            self.log_manager.log_statement(current_agent.name, response, metadata)
            history_log.append(f"{current_agent.name}: {response}")
            
            # 6. Detect natural meeting close from James during wrap-up
            if in_wrap_up and current_agent.name == "James":
                goodbye_phrases = [
                    "good discussion", "great discussion", "let's pick this up",
                    "let's reconvene", "meeting adjourned", "wrap it up",
                    "that's a wrap", "let's call it", "until next time",
                    "see you next time", "thanks everyone", "thank you everyone",
                    "good work today", "great work today", "let's end here",
                ]
                if any(phrase in response.lower() for phrase in goodbye_phrases):
                    self._run_farewell_round(topic, history_log)
                    break
        ...
        # If the farewell round didn't already happen (e.g. max_turns exhausted),
        # have James close and let the team say goodbye
        if not any("farewell_done" in h for h in history_log):
            print(f"\n\033[92mJames: Alright team, great discussion today! Let's reconvene soon.\033[0m")
            self.log_manager.log_statement("James", "Alright team, great discussion today! Let's reconvene soon.")
            history_log.append("James: Alright team, great discussion today! Let's reconvene soon.")
            self._run_farewell_round(topic, history_log)
    ...
    def _generate_agent_response(
        ...
        # Build a paper hint so agents know what's available
        paper_names = ", ".join(self.context_manager.paper_list[:6]) if self.context_manager.paper_list else "the loaded papers"
...
- Reference AT LEAST TWO different papers per response when the topic connects to multiple sources
- If you must speculate, prefix with [SPECULATION]
- Keep response under 150 words
- Do not use web searches; use only the loaded local research papers.

{agent.name}:"""

        self._animate_spinner(f"{agent.name} analyzing", duration=0.1, color=agent.color)
...
6. YOU MUST CITE at least one research paper by name (available: {paper_names})
7. Use "exact quotes" from the papers and tag with [from filename.md]

Your task: {mission}

Respond as {agent.name} only:"""
                elif turn_count == 0 and not history_log:
                    # True first turn of the meeting - open with introductions
...
2. MUST reference findings from AT LEAST TWO different papers to ground the discussion
3. Use "exact quotes" from the papers and tag with [from filename.md]
4. End with a question to spark discussion
5. Keep it under 100 words

Available papers: {paper_names}
Your specialty: {agent.focus}
Your task: {mission}

Respond as {agent.name} only:"""
                else:
                    # After FOUNDER intervention or own previous turn — continue naturally
                    user_msg = f"""LIVE TEAM MEETING - Your turn to speak.
...
=== CRITICAL RULES (MUST FOLLOW) ===
1. YOU ARE {agent.name.upper()} - Never speak as another person
2. DO NOT re-introduce yourself or the topic — the meeting is already underway
3. ADVANCE the discussion with a NEW point or question
4. Focus on YOUR specialty: {agent.focus}
5. Keep response under 80 words - be concise
6. YOU MUST CITE at least one research paper by name (available: {paper_names})
7. Use "exact quotes" from the papers and tag with [from filename.md]
...
Respond as {agent.name} only:"""
                ...
                # --- Inject hypergraph context for Luca & James ---
                graph_block = ""
                if self.hypergraph and agent.name in ("Luca", "James"):
                    graph_block = self.hypergraph.query(topic, depth=2, verbose=self.config.verbose)
                    if graph_block:
                        # Cap graph block to avoid blowing up the context window
                        if len(graph_block) > 2000:
                            graph_block = graph_block[:2000] + "\n[...graph results truncated...]"
                        graph_block = "\n\n" + graph_block

                # Build messages
                no_repeat = ""
                if turn_count > 0:
                    no_repeat = "\n\nIMPORTANT: The meeting is already underway. Do NOT re-introduce yourself or the topic. Jump straight into your point."
                
                # Inject rolling memory if available
                memory_block = ""
                if self.meeting_summary:
                    memory_block = f"\n\n### MEETING MEMORY (SUMMARY OF EARLIER DISCUSSION)\n{self.meeting_summary}"
                
                sys_content = f"{agent.soul}{no_repeat}{memory_block}\n\n### RESEARCH DATABASE\n{context_block}{graph_block}"
                messages = [
                    {"role": "system", "content": sys_content},
                    {"role": "user", "content": user_msg}
                ]

                # Diagnostic: show prompt size so we can catch overflows
                total_chars = len(sys_content) + len(user_msg)
                est_tokens = total_chars // 4  # rough estimate
                print(f"\033[90m   📏 Prompt: {total_chars:,} chars (~{est_tokens:,} tokens)\033[0m")
                    ...
                    for ri in range(self.config.recursive_depth):
                        ...
                        refined = self.client.chat.completions.create(
                            ...
                            messages=[
                                ...
                                {"role": "user", "content": (
                                    f"You are a strict research editor AND writer for {agent.name}.\n\n"
                                    f"DRAFT:\n{content}\n\n"
                                    "CRITIQUE this draft on: (1) factual grounding to papers, "
                                    "(2) novelty vs prior turns, (3) multi-paper coverage, (4) clarity.\n\n"
                                    f"Then REWRITE an improved version as {agent.name} in first person. "
                                    "Keep it under 80 words, cite at least two papers, avoid repetition.\n\n"
    ...
    def _run_farewell_round(self, topic: str, history_log: List[str]):
        """Generate a quick goodbye from every non-James agent after James closes the meeting."""
        print("\n\033[90m   --- Farewell round ---\033[0m")
        
        recent_chat = "\n".join(history_log[-4:]) if history_log else ""
        
        for agent in self.team:
            if agent.name == "James":
                continue  # James already said goodbye
            
            farewell_prompt = (
                f"The meeting about '{topic}' just ended. James said goodbye. "
                f"You are {agent.name}. Say a quick, natural goodbye to the team in ONE short sentence "
                f"(under 20 words). Stay in character. Be warm but brief — like leaving a meeting room."
            )
            ...
            try:
                ...
                bye_text = resp.choices[0].message.content.strip()
                
                # Clean up prefix if agent repeats their own name
                if bye_text.startswith(f"{agent.name}:"):
                    bye_text = bye_text[len(f"{agent.name}:"):].strip()
                
                print(f"{agent.color}   {agent.name}: {bye_text}\033[0m")
                if self.config.enable_voice:
                    self.voice_manager.speak(bye_text, agent.name)
                self.log_manager.log_statement(agent.name, bye_text)
                history_log.append(f"{agent.name}: {bye_text}")
            except Exception:
                # Fallback static goodbye if LLM call fails
                fallback = f"See you next time, team!"
                print(f"{agent.color}   {agent.name}: {fallback}\033[0m")
                history_log.append(f"{agent.name}: {fallback}")
        
        # Mark farewell as done so we don't repeat it
        history_log.append("[farewell_done]")

    def _get_wrap_up_instruction(self, agent: Agent, topic: str) -> str:
        """Get wrap-up instructions for each agent"""
        mission_instructions = {
            "James": f"""You are closing the meeting. As lead scientist:
...
Keep it under 60 words - maintain your standards but be collegial.""",
        ...
        return mission_instructions.get(agent.name, f"Provide your closing thoughts on '{topic}' in under 60 words.")
    ...
    def _load_previous_meeting(self):
        """Check meeting_archives for the most recent meeting log and offer to resume context."""
        archive_dir = os.path.join(self.config.library_path, "meeting_archives")
        ...
        try:
            choice = input(f"\n📂 Found previous meeting [{filename}]. Resume context? (y/n): ").strip().lower()
        except (EOFError, KeyboardInterrupt):
            choice = "n"
        ...
        if choice == "y":
            try:
                ...
                max_len = self.config.max_resume_context_length
                if len(previous_content) > max_len:
                    previous_content = "[...earlier discussion truncated...]\n" + previous_content[-max_len:]
                self.full_context += f"\n\n### PREVIOUS MEETING LOG\n{previous_content}"
                print(f"   ✓ Loaded {len(previous_content):,} chars from {filename}")
            except Exception as e:
                print(f"   ⚠️  Could not load previous meeting: {e}")
        else:
...
Examples:
  ...
  python rain_lab_v31_production.py --temp 0.3 --topic "entanglement"
    ...
    parser.add_argument(
        ...
        help='LLM read timeout in seconds (default: 300). Set to 0 or -1 for infinite.'
    )
    ...
    parser.add_argument(
        '--no-voice',
        action='store_true',
        help='Disable Text-to-Speech (TTS) output'
    )
