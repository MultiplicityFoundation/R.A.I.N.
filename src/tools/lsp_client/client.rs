...
impl LspClient {
    ...
    pub(crate) async fn document_symbols(
        &self,
        path: &Path,
    ) -> Result<Vec<DocumentSymbolEntry>, LspError> {
        ...
        let response = self
            ...
            .await?;
        Ok(symbol_response_to_entries(path, response))
    }
            ...
            let result = async {
                while let Some(message) = read_message(&mut reader).await? {
                    ...
                    if notification.diagnostics.is_empty() {
                        diagnostics_map.remove(notification.uri.as_str());
                    } else {
                        diagnostics_map
    ...
    async fn request<T>(&self, method: &'static str, params: Value) -> Result<T, LspError>
    where
        T: for<'de> serde::Deserialize<'de>,
    {
        ...
        let response = timeout(Duration::from_millis(REQUEST_TIMEOUT_MS), receiver)
            .await
            .map_err(|_| LspError::Timeout {
                operation: method,
