...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `R.A.I.N.` workspace
    Openclaw {
...
Examples:
  ...
  rain peripheral add esp32 /dev/ttyUSB0")]
    ...
    /// Flash R.A.I.N. firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash R.A.I.N. firmware to an Arduino board.
