...
use custom_url::parse_custom_provider_url;
use factory::{glm_base_url, minimax_base_url, moonshot_base_url, qwen_base_url, zai_base_url};
...
pub struct ProviderRuntimeOptions {
    ...
    /// These are merged from the config file and `rain_EXTRA_HEADERS` env var.
...
pub fn provider_runtime_options_from_config(
    ...
    ProviderRuntimeOptions {
        ...
        rain_dir: config.config_path.parent().map(PathBuf::from),
...
/// 3. Generic fallback variables (`rain_API_KEY`, `API_KEY`)
///
...
fn create_provider_with_url_and_options(
    ...
    match name {
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `rain onboard` to reconfigure.\n\
...
/// Return the list of all known providers for display in `R.A.I.N. providers list`.
///
...
mod tests {
    ...
    fn resolve_provider_credential_opencode_go_env() {
        ...
        let _rain_guard = EnvGuard::set("rain_API_KEY", None);
    ...
    fn provider_runtime_options_extra_headers_passed_through() {
        ...
        extra_headers.insert("X-Title".to_string(), "R.A.I.N.".to_string());
